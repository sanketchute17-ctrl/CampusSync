...
import { ArrowBigUp, MessageSquare, Ghost, User, Send, CheckCircle2, Sparkles, Loader2, X, ThumbsUp, ThumbsDown, Flag, AlertTriangle, Download, Bookmark } from 'lucide-react';
...
export default function DoubtCard({ doubt, currentUser, userProfile, isUserAnonymous, onRequireLogin }) {
  ...
  const [isSaved, setIsSaved] = useState(() => {
     try {
       const firestoreSaved = userProfile?.savedDoubts || [];
       const saved = localStorage.getItem('saved_doubts') || '[]';
       return firestoreSaved.includes(doubt.id?.toString()) || JSON.parse(saved).includes(doubt.id);
     } catch (e) {
       return false;
     }
  });
  ...
  const handleBookmark = async () => {
     ...
     try {
       let saved = JSON.parse(localStorage.getItem('saved_doubts') || '[]');
       if (!willSave) {
          saved = saved.filter(id => id !== doubt.id);
          toast("Removed from bookmarks");
       } else {
          if (!saved.includes(doubt.id)) saved.push(doubt.id);
          toast.success("Saved to your bookmarks! 📚");
       }
       localStorage.setItem('saved_doubts', JSON.stringify(saved));
       window.dispatchEvent(new Event('localBookmarksUpdated'));
     } catch(e) {}

     if (currentUser && currentUser.uid !== "anonymous_session") {
       try {
          const userRef = doc(db, 'users', currentUser.uid);
          if (willSave) {
             await updateDoc(userRef, { savedDoubts: arrayUnion(doubt.id.toString()) });
          } else {
             await updateDoc(userRef, { savedDoubts: arrayRemove(doubt.id.toString()) });
          }
       } catch (err) {
  ...
  const handleAIAnswer = async () => {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        toast.error("AI had a glitch.");
      } finally {
        setIsGeneratingAI(false);
        toast.success("AI generated an answer! ✨");
      }
    }, 1500);
  };
      ...
      <div className="flex flex-col items-center gap-3">
        <div className="flex flex-col items-center gap-1">
          <button 
            ...
            className={`p-1 rounded-full transition-colors ${hasUpvoted ? 'text-blue-600 bg-blue-50 dark:bg-blue-900/30' : 'text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700'}`}
          >
        ...
        <button 
           ...
           className={`p-1.5 rounded-full transition-all duration-300 ${isSaved ? 'text-yellow-500 bg-yellow-50 dark:bg-yellow-900/20 shadow-sm shadow-yellow-500/20 scale-110' : 'text-slate-300 hover:text-yellow-500 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
        >
